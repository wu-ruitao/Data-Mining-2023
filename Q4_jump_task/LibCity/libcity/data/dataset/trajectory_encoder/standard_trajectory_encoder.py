...
class StandardTrajectoryEncoder(AbstractTrajectoryEncoder):
    ...
    def __init__(self, config):
        ...
        self.feature_dict = {'history_loc': 'int', 'history_tim': 'int',
                             ...
                             'target': 'int', 'target_tim': 'int', 'uid': 'int', 'traj_id': 'int',
                             }
    ...
    def encode(self, uid, trajectories, negative_sample=None):
        ...
        """
        # 直接对 uid 进行重编码
        # uid = self.uid    # 此处不对 uid 进行重复编码，便于 test 时定位数据
        self.uid += 1
        encoded_trajectories = []
        encoded_trajectories_test = []
        history_loc = []
        history_tim = []
        for index, traj in enumerate(trajectories):
            ...
            if last_loc == -1:   # 测试数据 test data
                i = len(current_loc) - 2    # 直接取轨迹内的所有点
                trace = []
                ...
                trace.append(uid)
                trace.append(traj_id)

                encoded_trajectories_test.append(trace)
            else:
            ...
            if self.history_type == 'splice':
                ...
                history_loc.append(current_loc)
                history_tim.append(current_tim)
        # print(encoded_trajectories)
        return encoded_trajectories, encoded_trajectories_test
    ...
    def gen_data_feature(self):
        ...
        with open('/media/space/wrt/LibCity/location2id.json', 'w') as r:
            json.dump(self.location2id, r)
        with open('/media/space/wrt/LibCity/id2location.json', 'w') as r:
            json.dump(self.id2location, r)
