...
class TrajectoryDataset(AbstractDataset):
    ...
    def cutter_filter(self):
        ...
        if cut_method == 'time_interval':
            ...
            for uid in tqdm(user_set, desc="cut and filter trajectory"):
                ...
                for index, row in enumerate(usr_traj):
                    ...
                    if index == 0:
                        ...
                        if traj_id == prev_traj_id:
                            ...
                            if last_location == -1:# add into test
                                sessions.append(session)
                            else:
                                if len(session) >= min_session_len and train_count < max_sessions_len:
                                    sessions.append(session)
                                    train_count += 1
                            session = []
                            session.append(row.tolist())
                ...
                if last_location == -1:
                    sessions.append(session)
                elif len(session) >= min_session_len and train_count < max_sessions_len:
                    sessions.append(session)
    ...
    def encode_traj(self, data):
        ...
        for uid in tqdm(data, desc="encoding trajectory"):
            encoded_data[uid], encoded_data_test_return = self.encoder.encode(int(uid), data[uid])
            if len(encoded_data_test_return) != 0:
                encoded_data_test[uid] = encoded_data_test_return
    ...
    def divide_data(self):
        ...
        test_set = self.data['encoded_data_test'].keys()
        for uid in tqdm(user_set, desc="dividing data"):
            ...
            eval_data += encoded_trajectories[train_num:]

            # eval_data += encoded_trajectories[train_num:eval_num]
            # test_data += encoded_trajectories[eval_num:]

        for uid in tqdm(test_set, desc="dividing data"):
            encoded_trajectories_test = self.data['encoded_data_test'][uid]
            test_data += encoded_trajectories_test
