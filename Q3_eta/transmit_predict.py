...
# 读取CSV文件
df = pd.read_csv('eta_task.csv', parse_dates=['time'])

# 按照'traj_id'和'time'排序
df_sorted = df.sort_values(by=['traj_id', 'time'])

# 找到每个'traj_id'的最早和最晚时间的记录的索引
idx_first = df_sorted.groupby('traj_id')['time'].idxmin()
idx_last = df_sorted.groupby('traj_id')['time'].idxmax()

# 获取这些索引对应的行
df_filtered = df_sorted.loc[idx_first.drop_duplicates()]

# 在写入新文件之前，确保相同的traj_id是相邻的
#df_filtered_sorted = df_filtered.sort_values(by=['traj_id', 'time'])

traj_duration = df_filtered


# 获取每个 traj_id 最晚时间的索引
null_times = df_sorted['time'].isnull()

# 在null时间值的子集中进行分组，并找到每个组的索引
# apply函数将为每个组返回索引列表
idx_null_times = df_sorted[null_times].groupby('traj_id').apply(lambda x: x.index.tolist())
#print(idx_null_times)
flattened_idx = [idx for sublist in idx_null_times for idx in sublist]
# 利用上面找到的索引获取最晚时间的current_dis和coordinates值
df_latest = df_sorted.loc[flattened_idx, ['id','traj_id', 'current_dis', 'coordinates']]
print(df_latest['id'])
...
# 保存到新的CSV文件
traj_duration.to_csv('traj_to_predict.csv', index=False)